...
class DigitalTwinRuntime:
    def __init__(self, repository: KettleRepository | None) -> None:
...
def run_cli(runtime: DigitalTwinRuntime) -> None:
    help_text = (
        "Twin commands: status, plan <goal_id>, goal <goal_id> [temperature] [water_level], "
        "cmd on|off|temp <v>|water <v>, quit"
    )
    ...
    while True:
        ...
        if command == "goal" and len(parts) >= 2:
            params: Dict[str, Any] = {}
            if len(parts) >= 3:
                params["temperature"] = int(parts[2])
            if len(parts) >= 4:
                params["water_level"] = int(parts[3])
            runtime.execute_goal(parts[1], params)
            continue
        if command == "cmd" and len(parts) >= 2:
            dt_command = parts[1].lower()
            if dt_command == "on":
                runtime.execute_dt_command("turn_on")
                continue
            if dt_command == "off":
                runtime.execute_dt_command("turn_off")
                continue
            if dt_command == "temp" and len(parts) >= 3:
                runtime.execute_dt_command(
                    "set_target_temperature", {"temperature": int(parts[2])}
                )
                continue
            if dt_command == "water" and len(parts) >= 3:
                runtime.execute_dt_command(
                    "set_water_level", {"water_level": int(parts[2])}
                )
                continue
