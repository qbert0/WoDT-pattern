...
from smart_grinder_simulator.grinder_simulator.business_logic import GrinderBusinessLogic


class GrinderSimulator:
    def __init__(
        self,
        initial_state: GrinderState | None = None,
        grind_duration_seconds: int = 60,
        on_state_change: Optional[Callable[[dict], None]] = None,
    ) -> None:
        self.state = initial_state or GrinderState()
        self.logic = GrinderBusinessLogic(
            self.state,
            grind_duration_seconds=grind_duration_seconds,
        )
