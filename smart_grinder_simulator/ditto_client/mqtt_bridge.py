...
class GrinderMQTTBridge:
    def __init__(
        ...
        self.client = None
        self.state_topic = f"grinder/{self.thing_id}/state"
        self.command_topic = f"grinder/{self.thing_id}/commands"
        self.response_topic = f"grinder/{self.thing_id}/responses"
    ...
    def _on_message(self, client, _userdata, msg):
        try:
            ...
            if self.mode == "device" and msg.topic == self.command_topic:
                ...
                self.publish_response({"status": 200, "command": command, "result": result or {}})
    ...
    def publish_command(self, command: str, params: Dict[str, Any] | None = None) -> None:
        ...
        self.client.publish(
            self.command_topic,
            json.dumps({"command": command, "params": params or {}}),
            qos=1,
        )
        time.sleep(0.05)
